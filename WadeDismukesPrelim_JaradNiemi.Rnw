...
This is the relative frequency that we observe a gene tree given the species tree. One issue with this is that we have finitely many gene trees, and so the interpretation of the probability is suspect as we are, as in ()
...
To sample from $p(y^{rep} | y)$ the following steps are taken:
	\begin{enumerate}
		...
		\item Using the $(\mu, \sigma2)$ sample $n$ values of $y^{rep}$ from $N(y^{rep}_i;\mu,\sigma^2)$	
	\end{enumerate}
...
Define $x=((n-1)s^2 + n(\bar{y} - \mu)^2)/(2\sigma^2)$, then
		...
		\end{align}	
	Which is the kernel of the $t_{n-1}(\bar{y}, s^2 / n)$ distribution.
	
	Since we now now that $\mu \sim t_{n-1}(\bar{y}, s^2 / n)$ which is equivalent $\frac{\mu - \bar{y}}{s/\sqrt{n}}\sim t_{n-1}(0, 1) $ so if we draw samples from $p(y | \mu, \sigma^2)$ then $\frac{\bar{y} - \mu}{s/\sqrt{n}}\sim t_{n-1}(0, 1)$ which is the distribution of $T(y^{rep},\theta)$.
...
yrep <- rnorm(n, mu, sigma2)


test_quant <- function(yrep){
  ybar_rep <- mean(yrep)
  ybar_s <- sqrt(var(yrep))
  return(ybar_rep / (ybar_s / sqrt(length(yrep))))
}
...
empirical_test_quantity <- (ybar - 1) / (s/sqrt(n))

hist(tqs) + abline(v = empirical_test_quantity, col = "red")
@
